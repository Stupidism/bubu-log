...
if [[ -f "${BASE_ENV_FILE}" ]]; then
  awk '!/^(DATABASE_URL|DATABASE_URL_UNPOOLED|POSTGRES_URL|POSTGRES_PRISMA_URL|POSTGRES_URL_NON_POOLING|PAYLOAD_DATABASE_URL|PAYLOAD_DB_PUSH)=/' "${BASE_ENV_FILE}" > "${OUTPUT_FILE}"
else
  : > "${OUTPUT_FILE}"
fi

{
  ...
  echo "POSTGRES_PRISMA_URL=\"${POOLED_URL}\""
